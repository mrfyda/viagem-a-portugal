...
// routes.json is generated by tools/route-snapper: each same-chapter Stop
// pair's segment snapped to the roads of 1979 (motorways, IP/IC expressways
// and toll roads forbidden — see tools/route-snapper/README.md), keyed by
// the later Stop's ordinal and closed onto the exact Stop coordinates.
const snappedByOrdinal = new Map<number, number[][]>(
  routesData.segments.map((segment) => [segment.ordinal, segment.coordinates]),
);

/**
 * The six chapter Routes (ADR 0003), as one segment per consecutive Stop
 * pair — snapped to the period road network where the snapper could route
 * the pair, the straight line between the Stops otherwise. Per-segment
 * features let the timeline draw the journey progressively with a GPU
 * filter on `ordinal` (the segment's later Stop).
 */
    ...
    return [
      {
        ...
        geometry: {
          ...
          coordinates: snappedByOrdinal.get(stop.ordinal) ?? [

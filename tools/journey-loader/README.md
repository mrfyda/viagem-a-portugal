...
| 6 | `python3 tools/journey-loader/export_web.py --photos DIR --journey N --selection selection.txt` | AVIFs in `apps/blog/assets/viagem-N/` + `manifest.json` |
...
ImageIO, macOS 12+), `sips`, `python3`. No `pip`/`uv` packages needed — these
scripts use only the standard library, so they work even when PyPI is
unreachable. There is no local webp encoder, hence AVIF.
